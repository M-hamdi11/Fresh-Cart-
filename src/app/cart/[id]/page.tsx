    ...
    <div className="min-h-screen pb-20 bg-[#f5f5f0]">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/cart" className="text-sm text-green-600 font-semibold hover:text-green-700">
            ← Back to Cart
          </Link>
        </div>
        ...
        {Array.isArray(address) && address.map((addr: addressresponsedatatype) => (
          ...
          >
            <div className="flex gap-3">
              <div className={`mt-1 ${selectedAddress?._id === addr._id ? "text-green-600" : "text-gray-400"}`}>
                📍
              </div>
              <div>
            ...
            {selectedAddress?._id === addr._id && (
              <div className="w-6 h-6 bg-green-600 rounded-full flex items-center justify-center text-white text-[10px] shadow-sm">
                ✓
              </div>
            )}
          </div>
      ...
      <div className="p-4 bg-gray-50 border-t flex justify-center">
        ...
        >
          + Add New Address in Profile
                ...
                {/* THE FORM - FIXED: Added id="checkout-form" */}
                <form id="checkout-form" onSubmit={handleSubmit(mysubmit)} className="space-y-4 pt-2">
                  ...
                  <div>
                    ...
                    <textarea
                      {...register("details")}
                      className={`w-full border rounded-xl px-4 py-3 text-sm focus:ring-2 outline-none resize-none transition ${
                  ...
                  <div>
                    ...
                    {errors.phone && (
                     <p style={{ color: 'red', fontWeight: 'bold' , fontSize: '12px', marginTop: '5px',paddingLeft: '5px'}}>{errors.phone.message}</p>
                    )}
                  </div>
                </form>
              </div>
          ...
          {/* ===== RIGHT: Order Summary ===== */}
          <div style={{ width: 320, flexShrink: 0, position: "sticky", top: 32, alignSelf: "flex-start" }}>
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-md">
              ...
              {/* Items List */}
              <div className="p-4 space-y-3 border-b border-gray-100 max-h-60 overflow-y-auto">
                {data?.products?.map((item: CartItemType, i: number) => (
                  <div key={i} className="flex gap-3 items-center">
                    <img src={item.product.imageCover} className="w-10 h-10 object-contain bg-gray-50 rounded border" />
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] font-bold text-gray-800 line-clamp-1">{item.product.title}</p>
                      <p className="text-[10px] text-gray-400">{item.count} × {item.price} EGP</p>
                    </div>
                    <p className="text-xs font-bold text-gray-900">{item.count * item.price}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="p-5 bg-gray-50 space-y-3">
                ...
                <button
                  type="submit"
                  form="checkout-form"
                  className="w-full bg-green-600 hover:bg-green-700 active:scale-95 text-white py-4 rounded-xl font-bold text-sm transition-all shadow-lg"
                >
                  Place Order
                </button>
              </div>
